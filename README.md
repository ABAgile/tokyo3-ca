...
```sh
make docker-up                       # shared mesh: backplane + certd + split cert-agentd agents
...
interactions. For local development, CA's split cert-agentd agents write
service workload certs into `tokyo3_shared_data:/workloads` and seed separate
agent state under `tokyo3_shared_data:/agent/authd` and `/agent/vaultd`;
downstream stacks mount that volume read-only and do not need their own
cert-agentd sidecar. The shared volume contains
...
shared/
  ...
  agent/
    authd-workloads.yml   # authd cert-agentd workload manifest
    vaultd-workloads.yml  # vaultd cert-agentd workload manifest
...
- `tokyo3_shared_data` — downstream-facing volume containing `/certs`
  ...
  CA's split cert-agentd agents. Sibling repos mount it read-only without seeing
  CA-local policy or config. `shared/certs/` still contains dev bootstrap
  private keys, so treat it as local-dev material.

Each cert-agentd self-seeds its own directory on first boot from its
bootstrap SVID, the CA trust bundle, and its source workload manifest. The
runtime state is `/tokyo3/agent/authd` or `/tokyo3/agent/vaultd`, while issued
certs go to `/tokyo3/workloads`; re-running `_sync-shared` never clobbers the
runtime directories. The runtime paths are
...
(`CERT_AGENTD_WORKLOAD_CA=/tokyo3/agent/<owner>/trust-bundle.pem`). The one mkcert
...
`shared/policy/principals.json` (`CERTD_MTLS_PRINCIPALS_FILE`) maps
`spiffe://tokyo3/authd/agent` to `["authd"]` and
`spiffe://tokyo3/vaultd/agent` to `["vaultd"]`. Each agent's bootstrap and
renewed certs carry its own SAN, so it authenticates as the same principal on
the first request and every renewal — `CERT_AGENTD_GROUPS` is now an inert
fallback (body-groups aren't consulted once principals are wired). The authd
agent provisions authd's workload certs from
...
normal certificate reload mechanism. The same agent also provisions
Vault's `vaultd` server, DB app/admin, and NATS identities under
`spiffe://tokyo3/vaultd/*`; Vault integration can consume them from
`/tokyo3/workloads` in a later mesh compose update. Both `roles.json` and
`principals.json` *seed* the Postgres store on first boot. OIDC stays off (no human
...
```sh
docker compose logs -f cert-agentd-authd cert-agentd-vaultd
docker compose exec cert-agentd-authd ls -l /tokyo3/workloads   # authd-*.crt/key
...
docker compose exec cert-agentd-authd sh -c \
...
(the rig points each agent at its own `/tokyo3/agent/*/trust-bundle.pem`)
and rewrites the
...
```sh
docker compose exec cert-agentd-authd \
...
- **Split agents** `cert-agentd-authd` and `cert-agentd-vaultd` keep
  workload issuance identities and manifests isolated while sharing the CA
  and output volume.
