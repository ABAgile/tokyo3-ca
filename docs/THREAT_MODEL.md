...
| 6 | Cross-site forgery against the portal                                               | `checkCSRF` (`portal/csrf.go`) enforces session-bound tokens (HMAC over a secret sealed into the session cookie — the OIDC login session, or an anonymous CSRF-carrier session under Basic auth; see `base/csrf`) on every POST. SameSite=Lax + constant-time compare. Tests in `csrf_test.go`.                                                                              |
...
| 2 | CA key compromise via process memory dump (in-memory)   | Accepted residual risk in dev. Mitigation in prod is `RemoteSigner`. The plan calls for `mlock`-style protection on the in-memory path — NOT IMPLEMENTED; tracked as future work. The two-tier intermediate (see #5) is unsealed into this same in-memory path (and the decrypted PKCS#8 buffer is not zeroized), so it inherits this risk — bounded there by the intermediate's short lifetime + cheap rotation. |
...
| 5 | Online exposure of the signing key on the issuance hot path | Optional **two-tier** mode ([two-tier-ca.md](two-tier-ca.md)): the asymmetric **root** stays offline and signs only the intermediate; certd signs leaves with a short-lived **intermediate** key sealed under a symmetric KMS key (`CERTD_CA_SEALED_KEY_FILE` + `CERTD_CA_SEAL_KEY`) and unsealed into memory at boot. A certd/IAM compromise then reaches only the intermediate — bounded by its lifetime and rotated without touching the root or any consumer anchor (consumers pin the root via `CERTD_CA_ROOT_CERT_FILE`) — not the root. No X.509 CRL, so the intermediate lifetime *is* the containment window: keep it short (≈90d), or use local-hardware key custody (TPM/enclave). Single-tier remains the default; this is opt-in. |
