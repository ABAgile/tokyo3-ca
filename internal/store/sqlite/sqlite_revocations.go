	...
	return inTx(s.db, func(tx *sql.Tx) error {
		if r.Serial != 0 {
			if _, err := tx.ExecContext(context.Background(),
				`DELETE FROM ssh_revocations WHERE serial = ?`, strconv.FormatUint(r.Serial, 10)); err != nil {
				return err
			}
		}
		if r.KeyID != "" {
			if _, err := tx.ExecContext(context.Background(),
				`DELETE FROM ssh_revocations WHERE key_id = ?`, r.KeyID); err != nil {
				return err
			}
		}
		_, err := tx.ExecContext(context.Background(),
			`INSERT INTO ssh_revocations (`+store.RevocationColumns+`) VALUES (?, ?, ?, ?, ?)`,
...
// on error. (The numeric serial ordering the KRL spec needs is applied by
...
func (s *revocationStore) Snapshot() krl.Snapshot {
	...
	if err != nil {
		s.log.Error("sqlite revocations Snapshot query failed", "err", err)
		return krl.Snapshot{CapturedAt: now}
	}
	defer rows.Close()
	entries, err := store.ScanRevocations(rows)
	if err != nil {
		s.log.Error("sqlite revocations Snapshot scan failed", "err", err)
		return krl.Snapshot{CapturedAt: now}
	}
...
func (s *revocationStore) IsRevoked(serial uint64, keyID string) bool {
	...
	case errors.Is(err, sql.ErrNoRows):
		return false
	case err != nil:
		s.log.Error("sqlite IsRevoked failed; failing closed (treating as revoked)", "err", err)
		return true
	}
	return true
}
