...
func (s *principalStore) Lookup(sans []string) (*mtls.Principal, error) {
	...
	for _, san := range sans {
		row := s.db.QueryRowContext(context.Background(),
			`SELECT `+store.PrincipalColumns+` FROM principals WHERE san = $1`, san)
		p, err := store.ScanPrincipal(row)
		switch {
		case errors.Is(err, sql.ErrNoRows):
			continue
		case err != nil:
			s.log.Error("postgres principal Lookup failed", "san", san, "err", err)
			return nil, err
		}
		return &p, nil
	}
...
func (s *principalStore) All() []mtls.Principal {
	...
	if err != nil {
		s.log.Error("postgres principals All query failed", "err", err)
		return nil
	}
	defer rows.Close()
	out, err := store.ScanPrincipals(rows)
	if err != nil {
		s.log.Error("postgres principals All scan failed", "err", err)
		return nil
	}
	return out
}
...
func (s *principalStore) BySAN(san string) (mtls.Principal, bool) {
	...
	case err != nil:
		s.log.Error("postgres principal BySAN failed", "san", san, "err", err)
		return mtls.Principal{}, false
	}
	return p, true
}
...
func (s *principalStore) Replace(oldSAN string, p mtls.Principal, source string) error {
	...
	return inTx(s.db, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(context.Background(), `DELETE FROM principals WHERE san = $1`, oldSAN); err != nil {
			return err
		}
		return insertPrincipal(tx, p, source)
	})
...
func (s *principalStore) Delete(san string) error {
	res, err := s.db.ExecContext(context.Background(), `DELETE FROM principals WHERE san = $1`, san)
	if err != nil {
		return err
	}
...
func principalExists(tx *sql.Tx, san string) (bool, error) {
	var one int
	err := tx.QueryRowContext(context.Background(), `SELECT 1 FROM principals WHERE san = $1`, san).Scan(&one)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		return false, nil
	case err != nil:
...
func insertPrincipal(tx *sql.Tx, p mtls.Principal, source string) error {
	...
	_, err = tx.ExecContext(context.Background(),
		`INSERT INTO principals (`+store.PrincipalColumns+`, updated_at, source) VALUES ($1, $2, $3, $4, $5)`,
		args...)
	return err
}
